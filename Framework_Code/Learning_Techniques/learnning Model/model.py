        ...
        for s in range(NUM_SCALES):
          ...
          for i in range(self.batch_size):
            ...
            for j in range(self.seq_length):
              ...
              #object_masks_warped = object_masks_warped.set_shape([None, self.img_height, self.img_width, 3])


              object_masks_warped = tf.map_fn(  # (N, H, W, 3)
                  process_obj_mask_warp, object_ids, dtype=tf.float32)

              object_masks_warped.set_shape([None, self.img_height, self.img_width, 3])
