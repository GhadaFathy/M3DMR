...
OUTPUT_DIR = '/Users/Ghada/PHD/Mask-processed/'
...
for d in glob.glob(INPUT_DIR + '/*/'):
    file = open("/media/ferdyan/NewDisk/Mask/txt_hasil/train.txt", "w")
    date = d.split('/')[-2]
    ...
    for d2 in glob.glob(d + '*/'):
        ...
        for subfolder in ['images']:
            ...
            ct = 1
            seqname = d2.split('/')[-2] + subfolder.replace('images', '')
            ...
            calib_camera = calib_raw[0] if subfolder=='images' else calib_raw[1]
            folder = d2 + subfolder
            ...
            for i in range(SEQ_LENGTH, len(files)+1, STEPSIZE):
                ...
                file.write(OUTPUT_DIR + seqname + '/' + ' ' + imgnum + '.png' + '\n')
